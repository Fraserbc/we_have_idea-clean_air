...
for col in collumns:
	...
	cur.execute("SELECT {},location FROM data".format(col))
	data = cur.fetchall()
	# print(data, col)
	...
	for x in range(len(data)):
		if data[x][0] == None:
			rows_to_update.append( data[x][1] )
		else:
...
# Add the lat long to the db
lat_lon = {
	"Edinburgh Currie":[55.8989, -3.3110],
	"Edinburgh Gorgie Road":[55.935361, -3.242611],
	"Edinburgh Nicolson Street":[55.945597, -3.184575],
	"Edinburgh Queensferry Road":[55.960734, -3.294549],
	"Edinburgh Salamander St":[55.973640, -3.157876],
	"Edinburgh St John's Road":[55.942914, -3.283028],
	"Edinburgh St Leonards":[55.949682, -3.179732],
	"Edinburgh Tower Street":[55.977090, -3.166172]
}

# GO through every row and add it
for row in lat_lon.keys():
	lat, lon = lat_lon[row]

	query = "UPDATE data SET lat={}, lon={} WHERE location=\"{}\"".format(lat, lon, row)
	cur.execute(query)
